    ...
    public static class WithdrawPresenter
    {
        ...
        private static void getWithdrawTryskins(int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                Withdraw.name.Add(name_value.Text);
        ...
        public static void createWithdraw()
        {
            try
            {
                ...
                for (int i = 0; i < Withdraw.name.Count(); i++)
                {
                    mainForm.Invoke(new MethodInvoker(delegate {
                        mainForm.withdraw_dataGridView.Rows.Add();
                        mainForm.withdraw_dataGridView.Rows[i].Cells[1].Value = Withdraw.name[i];
                    ...
                    if (Withdraw.sales[i] > 2000) mainForm.withdraw_dataGridView.Invoke(new Action(() => { mainForm.withdraw_dataGridView.Rows[i].Cells[4].Style.BackColor = Color.Green; }));
